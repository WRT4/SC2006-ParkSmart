...
export default class SearchPageController {
  ...
  fetchCarparkAvailability = ({
    ...
    fetch("http://localhost:5000/api/carparks")
    .then((res) => res.json())
    .then((data) => {
      setAvail(data);
      if (records.length > 0) {
        setRecords(
          records.map((rec) => {
            const carpark = data.find((el) => el.carpark_number === rec.car_park_no);
            if (carpark) {
    ...
    .catch((err) => console.error("Error loading carpark data:", err));
  };
  ...
  firstLoadFetch = (
    ...
    fetch("http://localhost:5000/api/records")
      ...
      .then((records) => {
        fetch("http://localhost:5000/api/carparks", { method: "GET", mode: "cors" })
          .then((response) => response.json())
          .then((data) => {
